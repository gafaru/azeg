...
for resource in resources:
    ...
    if resource['type'] == 'azurerm_eventgrid_domain':
        ...
        if 'domain_topic_name' in resource['values']:
            ...
            # Iterate over the domain topics and look for the one with the matching name
            for domain_topic in domain_topics:
                if domain_topic.name == domain_topic_name:
                    # Get the properties of the domain topic
                    domain_topic_resource_id = domain_topic.id
                    domain_topic_resource_group = domain_topic.id.split('/')[4]
                    domain_topic_name = domain_topic.name
                    
                    # Print the domain topic properties
                    print(f'\tAzure Event Grid domain topic "{domain_topic_name}" found with resource ID "{domain_topic_resource_id}"')
                    print(f'\tResource group: {domain_topic_resource_group}')
                    
                    # Get the event subscriptions associated with the domain topic
                    event_subscriptions = eventgrid_client.event_subscriptions.list_by_domain_topic(domain_resource_id, domain_topic_name)
                    
                    # Iterate over the event subscriptions and validate that they exist
                    for subscription in event_subscriptions:
                        if subscription.provisioning_state == 'Succeeded':
                            print(f'\tEvent subscription "{subscription.name}" found with ID "{subscription.id}"')
                        else:
                            print(f'\tEvent subscription "{subscription.name}" has not been provisioned successfully')
                    
                    break
            else:
                print(f'\tNo domain topic found with name "{domain_topic_name}"')
        else:
            print(f'\tDomain "{domain_name}" does not have a domain topic')
